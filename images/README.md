...
## ロゴデザインの意図（2026-05-06 第2版）

親会社・日本荷造運送株式会社のロゴ（[ロゴ決定.pdf](../../system_update/0_website/ロゴ決定.pdf) 参照）の**意味構造**ごと継承して再設計。

親会社ロゴの真意は「**人が重いものを持ち上げている**」:
- 中央のオレンジ菱形リング = 人の体・両腕（広げた姿）
- 中央のオレンジ円 = 頭
- 上の黒菱形 = 持ち上げている重いもの（荷物）
- 下の黒菱形 = 足元・接地点

新ロゴはこのモチーフを「**人がPCを操っている**」に変換:

- **中央のオレンジ菱形リング** — 親会社からそのまま継承（人の体・両腕）。DIC-124（≒ #F4A933）
- **中央のシアン点（#00d4ff）** — 人の頭。親会社のオレンジ円をシアンに置換し、新事業（IT×AI）のテックサインを兼ねる
- **上の黒菱形＋内部のシアン2本線** — 親会社の「重いもの」を「**画面のあるもの＝PC**」に意味変換。内部のシアン2本線が画面に表示されているテキスト行を象徴し、「人がPCを操っている」が直接伝わる
- **下の黒菱形** — 親会社からそのまま継承（足元・接地点）

サイトのダーク背景上では上下の菱形は `currentColor` でテキスト色（オフホワイト）を継承し、ファビコンは `prefers-color-scheme` で明暗を切り替える。

設計思想: **親会社ロゴの意味構造を維持しつつ、操作対象だけを「荷物→PC」に置き換えることで、運送業からIT支援への進化を1点突破で表現する**。
